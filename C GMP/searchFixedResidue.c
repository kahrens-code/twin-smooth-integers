...
int main() {
	...
	printf("Find twin smooth integers with fixed residue approach\n");

//Initialize and set parameters and variables
	printf("\nInitalize and set\n");

	//general parameters
	mpz_t start; mpz_init(start); mpz_ui_pow_ui(start, 2, 77);	//start of search interval I
	mpz_t temp; mpz_init(temp);
	mpz_set_ui(temp, 5771633313600);	//C for n7ex5
	mpz_mul_ui(temp, temp, 1024);	//numberBatches
	mpz_mul_ui(temp, temp, 2097152);	//batchLength for this setup
	mpz_mul_ui(temp, temp, 47);	//iteration
	mpz_add(start, start, temp);
	mpz_ui_pow_ui(temp, 2, 27);
	...
	unsigned int numberBatches = 1024;//32768 (2^15);	//number of batches for full test
	...
	unsigned short chooseFixedResidues = 1;	//set mode for choosing fixedResidue
...
//Pre-computations for smoothness tests
	...
	if (mpz_fits_ulong_p(factorSum) && togglePrimeResidues == 1) {
		...
		if (mpz_fits_ulong_p(numberResidues) && toggleResidues == 1) {
			...
			if (toggleResidues != 1) {
				printf("Warning: computation of residues was toggled off\n");
			} else {
				printf("Warning: number of residues mod C does not fit in unsigned long. Can not allocate list of residues.\n");
			}
		}
	}else {
		if (togglePrimeResidues != 1) {
			printf("\nWarning: computation of primeResidues was toggled off\n");
		} else {
			printf("\nWarning: number of possible residues mod prime power factors of C does not fit in unsigned long. Can not allocate list of primeResidues.\n");
		}
	}
	finalPre = time(NULL);
	...
	gmp_printf("number of rounds: %u, end: %Zd (size in base 2: %lu)\n", numberBatches, temp, mpz_sizeinbase(temp, 2));
	srand(time(NULL));
