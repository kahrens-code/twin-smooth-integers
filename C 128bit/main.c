...
//general parameters
	...
	unsigned int size = pow2(20);		//size of chunks
	...
	unsigned __int128 start = pow2(80);		//start of search interval I
	unsigned __int128 totalSize = 1000000000;	//size of the whole search interval
	unsigned __int128 end = start + totalSize;	//end of search interval I
	unsigned int smoothnessBound = pow2(16);	//smoothness bound
...
//set PTE specific parameters	
	unsigned short startParameter = 0;	//0: not changed, >0: set to specific start value
	setParameters(&n8ex1, &degree, &numberRoots, &roots, &polyA, &polyB, &C, &start, startParameter, totalSize, &end);
...
//variables
	unsigned __int128 currentStart;	//start of current chunk
	...
	unsigned __int128 *logTable;		//array with thresholds for rounded logarithms
	...
	#endif
	unsigned __int128 *smoothInterval;	//auxiliary array for regular sieve
	unsigned short *smoothNumbers;	//array of 1s and 0s for smooth and non-smooth integers, respectively
	unsigned long numberResidues;		//number |R| of residue classes r mod C s.t. a(r)/C and b(r)/C are integers
	unsigned long *residues;		//pointer to array of such classes
	...
	unsigned __int128 *smoothIntsModC;	//pointer to array of such elements
...
//print results on the screen
	...
	printf(", residues: %lu", numberResidues);
